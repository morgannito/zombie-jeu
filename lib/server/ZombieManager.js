 ...
 * - Plafonnement à la vague 100 pour les calculs de difficulté
 * - Empêche les valeurs démesurées qui causent des crashs serveur
 * - Au-delà de la vague 100, la difficulté reste constante (très difficile)
 *
 * @version 1.1.0
 */
...
class ZombieManager {
  ...
  spawnSingleZombie() {
    ...
    // CORRECTION: Plafonner l'escalade à la vague 100 pour éviter les valeurs démesurées
    const effectiveWave = Math.min(this.gameState.wave, 100);
  ...
  spawnBoss() {
    ...
    // Le boss devient plus fort à chaque vague avec PLAFOND
    // CORRECTION: Plafonner l'escalade à la vague 100 pour éviter les valeurs démesurées
    const effectiveWave = Math.min(this.gameState.wave, 100);
