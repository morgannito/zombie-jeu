...
class ZombieManager {
   ...
   */
  spawnBoss() {
    // Vérifier si c'est une vague de boss spécial
    ...
    if (this.gameState.wave === 25) {
      ...
      isSpecialBoss = true;
    } else if (this.gameState.wave === 50) {
      type = this.zombieTypes.bossInfect;
      bossType = 'bossInfect';
      isSpecialBoss = true;
    } else if (this.gameState.wave === 75) {
      type = this.zombieTypes.bossColosse;
      bossType = 'bossColosse';
      isSpecialBoss = true;
    } else if (this.gameState.wave === 100) {
      type = this.zombieTypes.bossRoi;
      bossType = 'bossRoi';
      isSpecialBoss = true;
    } else if (this.gameState.wave === 130) {
      type = this.zombieTypes.bossOmega;
      bossType = 'bossOmega';
      isSpecialBoss = true;
    }

    // Boss normaux: scaling avec wave multiplier
    let bossHealth, bossDamage, bossGold, bossXP;

    if (isSpecialBoss) {
      // Boss spéciaux: stats fixes (pas de wave multiplier)
      bossHealth = type.health;
      bossDamage = type.damage;
      bossGold = type.gold;
      bossXP = type.xp;
    } else {
      // Boss normaux: scaling progressif avec PLAFOND
      const effectiveWave = Math.min(this.gameState.wave, 130);
